...
from common.trajectory import get_tid, Trajectory, STPoint
...
def split_data(traj_input_dir, output_dir, raw2new_rid_dict):
    """
    ...
    trg_parser = ParseMMTraj(raw2new_rid_dict)
            ...
            if len(path) > 1:
                for seg_id in range(len(path) - 1):
                    ...
                    if (seg_id == len(path) - 2) and (dest_pt.rate == 0):
                        ...
                        seg_length = self.rn[node_start][node_end]['length']
                        seconds = seg_length / velocity
                        time = time + timedelta(seconds=seconds)
                    lat = node_end[1]
                    lng = node_end[0]
                    traj_enhanced.append(STPoint(lat, lng, time))
